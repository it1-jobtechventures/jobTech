...
//         <PdfShow url={url} sequence={1}/>
//         <ExcelViwer url={url}/>
//         <PdfShow url={url} sequence={2}/>
//         <Video url={url}/>
//         <PdfShow url={url} sequence={3}/>
//         </div>
...
const ContactForm = ({url}) => {
  ...
  const displayPdf = async () => {
    try {
      ...
      const sortedData = res.data.data.sort((a, b) => a.sequence - b.sequence);
      setData(sortedData);
    } catch (error) {
  ...
  return (
    <div className=" py-12 pt-24 " id='section6'>
      <div className="container mx-auto px-4 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Video url={url}/>
        {data.map((pdf) => (
          <PdfShow key={pdf._id} url={url} sequence={pdf.sequence} title={pdf.title}/>
        ))}
        </div>
